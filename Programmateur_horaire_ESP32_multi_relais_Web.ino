...
/* 🚩 Dimension Switch Auto Manuel voir 2️⃣ dans Infos.ino*/
...
.toggle .track::before{content:""; position:absolute; height:17px; width:17px; left:4px; top:0px; background:#fff; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.4);}
...
async function update() {
  try {
    ...
    RELAYS.forEach(({ id: p }) => {
      const d = data[p];
      document.getElementById(p + 'mode-text').innerText = d.auto ? "AUTO ⏱️" : "MANUEL";
      document.getElementById(p + 'auto-switch').checked = d.auto;
...
// 1️⃣Bascule AUTO/MANUEL : on attend la confirmation du serveur avant de
...
void loop() {
  ...
  if (millis() - lastCheck >= 1000) {
    ...
    // affichera toujours la même page unique).
