...
//  (unique, sans espace, ex: "Cl") et la broche GPIO. Pour en retirer un,
...
.remain{font-size:.78rem; text-align:center; margin:4px 0 0; min-height:14px;}
...
if (d.auto && data.actuelle && data.actuelle !== '--:--') {
  ...
  if (d.etat) {
    remainEl.style.color = "#FF8787"; // Rouge vif (plus lisible sur fond sombre que le rouge pur)
    remainEl.innerText = "⏳ Extinction dans " + formatRemainMin(remain);
  } else {
    remainEl.style.color = "#2ecc71"; // Vert agréable
    remainEl.innerText = "⏳ Allumage dans " + formatRemainMin(remain);
  }
