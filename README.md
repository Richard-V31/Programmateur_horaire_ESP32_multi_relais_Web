...
- 4 **boutons poussoirs** (optionnels) pour le forçage physique des relais PR1 à PR4 — aucune résistance externe nécessaire (pull-up interne activé par le programme).
...
### Broches GPIO des boutons poussoirs de forçage (PR1 à PR4)
...
Créez, à côté du fichier `.ino`, un fichier `arduino_secrets.h` (non fourni et à ne pas partager) contenant au minimum :
...
Programmateur programmateurs[] = {
  { "PR1", "Programmation 1", "Cuisine",  "#f59e0b", 32, "08:00", "18:00", true, false, 14 },
  { "PR2", "Programmation 2", "Portail",  "#06b6d4", 33, "09:00", "19:00", true, false, 16 },
...
| `pinBP` | Broche GPIO du bouton poussoir de forçage physique (câblé entre GND et cette broche). Mettre `-1` si ce relais n'a pas de bouton câblé. Dans le programme fourni, seuls PR1 à PR4 en ont un (GPIO 14/16/17/18) |
...
> 🔘 Pour PR1 à PR4, le même forçage ON/OFF peut aussi être déclenché directement par le bouton poussoir physique associé (voir câblage plus haut), sans passer par la page web. L'action se reflète immédiatement dans l'interface et sur l'écran OLED.
...
- À **chaque passage** de `loop()` (sans délai), l'état des boutons poussoirs de forçage (PR1 à PR4) est vérifié avec anti-rebond logiciel (40 ms) : un appui bascule immédiatement le relais concerné en mode manuel et inverse son état, puis sauvegarde en NVS.
...
- Le forçage par bouton poussoir n'est câblé, dans le programme fourni, que pour PR1 à PR4 (GPIO 14/16/17/18) ; il peut être étendu à d'autres relais en renseignant leur champ `pinBP` (voir configuration ci-dessus).
